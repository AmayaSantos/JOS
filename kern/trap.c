...
trap_init_percpu(void)
{
	...
	// '''' trap_init_percpu
	struct TaskState *thiscpu_ts = thiscpu->cput_ts;
	int thiscpu_id = thiscpu->cpu_id;

	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	thiscpu_ts.ts_esp0 = KSTACKTOP - thiscpu_id * (KSTACKTOP - KSTACKGAP);
	thiscpu_ts.ts_ss0 = GD_KD;
	thiscpu_ts.ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	uint16_t idx = (GD_TSS0 >> 3) + thiscpu_id;
	uint16_t seg = idx << 3;

	gdt[idx] =
			SEG16(STS_T32A, (uint32_t)(&thiscpu_ts), sizeof(struct Taskstate) - 1, 0);
	gdt[idx].sd_s = 0;
